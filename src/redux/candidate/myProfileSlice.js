...
export const getProfile = createAsyncThunk('getProfile', async (payload, { dispatch }) => {
...
export const getNationality = createAsyncThunk('getNationality', async (payload, { dispatch }) => {
...
export const getLanguage = createAsyncThunk('getLanguage', async (payload, { dispatch }) => {
    const { data } = await getApi('/languages', true)
    return data
})

export const createProfile = createAsyncThunk('createProfile', async (payload, { dispatch }) => {
...
export const uploadProfilePic = createAsyncThunk('uploadProfilePic', async (payload, { dispatch }) => {
    dispatch(setLoading(true))
    const { data } = await postApi('/upload/profilepic', payload, true, "multipart/form-data") //"multipart/form-data"
    dispatch(setLoading(false))
    return data
})
