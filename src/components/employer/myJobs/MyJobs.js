...
import { getTalentJobStatusCount } from "../../../redux/employer/myJobsSlice";
...
export default function MyJobs() {
  ...
  // const allEmployeeJobs = useSelector(
  //   (state) => state.configemployerjobs.employerjobs
  // );
  ...
  const [talentJobStatus, settalentJobStatus] = useState([]);
  ...
  const getTalentMyJobStatusCount = async (jobId) => {
    try {
      ...
      settalentJobStatus(
        manage.payload.data?.map((item) => ({ ...item, items: [] }))
      );
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <ManageJob
          talentJobStatus={talentJobStatus}
          settalentJobStatus={settalentJobStatus}
          jobId={jobId}
        />
      </CustomDialog>
    </Grid>
