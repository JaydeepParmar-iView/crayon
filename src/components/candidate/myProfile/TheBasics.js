...
export default function TheBasics({ handleProfileData, profile }) {
  ...
  const [characterCount, setCharacterCount] = useState(profileData.my_bio?.length || 0);
  ...
  const handleDateChange = (newValue)=>{
    const newProfileData = {
      ...profileData,
      dob: dayjs(newValue).format("MM/DD/YYYY"),
    };
    setProfileData(newProfileData);
    handleProfileData(newProfileData);
  }
  ...
  return (
    <Box sx={{ mt: 3 }}>
      ...
      >
        <Box sx={{ width: "50%" }}>
          ...
          <InputBox
            ...
            value={profileData.contact_no ? parseInt(profileData.contact_no) : ""}
            id="contact_no"
            onChange={handleInputChange}
          />
        </Box>
        ...
        <Box sx={{ width: "50%", display: "flex",justifyContent:"space-between", alignItems: "center" }}>
          <Box sx={{ width: "50%" }}>
            ...
            <SelectMenu
              ...
              value={towns.find((val)=> val.town_id == profileData.town_id)?.name || ""}
      ...
      <Box sx={{ display: "flex", mb: 4, ml: 1,justifyContent: "space-between",}}>
        <Box sx={{ width: "47%",}}>
